"""
CuraNex AI — Feature Engineering Pipeline
=============================================
...
- SKU metadata features: category, group, dosage_group, price, country encodings
- External signals: holiday/festival flags, health indices + rainfall
...
def add_retailer_features(df: pd.DataFrame, retailers: pd.DataFrame, transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Ensure retailer_id types match
    df["retailer_id"] = df["retailer_id"].astype(str)
    retailers = retailers.copy()
    retailers["retailer_id"] = retailers["retailer_id"].astype(str)

    # Merge static retailer attributes (include retailer_name for display)
    merge_cols = ["retailer_id", "retailer_name", "retailer_type", "district", "tier", "years_active"]

    df = df.merge(
        retailers[merge_cols],
        on="retailer_id",
        how="left",
    )
    ...
    rfm["retailer_id"] = rfm["retailer_id"].astype(str)
...
def add_sku_features(df: pd.DataFrame, skus: pd.DataFrame) -> pd.DataFrame:
    ...
    # Columns to merge
    sku_merge_cols = [
        "sku_id", "brand", "therapeutic_category", "group", "group_description",
        "dosage_form", "dosage_group", "price_band", "is_critical",
        "shelf_life_months", "supplier_lead_time_days", "country",
        "schedule", "validation", "reg_date",
    ]
    available_cols = [c for c in sku_merge_cols if c in skus.columns]
    ...
    # Compute product_age_weeks (from reg_date to transaction date)
    if "reg_date" in df.columns:
        df["reg_date"] = pd.to_datetime(df["reg_date"])
        df["product_age_weeks"] = ((df["date"] - df["reg_date"]).dt.days / 7).clip(lower=0).fillna(0)
    else:
        df["product_age_weeks"] = 0

    # Encode categoricals
    for col in ["therapeutic_category", "group", "dosage_group", "price_band", "country", "schedule"]:
...
def add_health_features(df: pd.DataFrame, health_signals: pd.DataFrame) -> pd.DataFrame:
    ...
    health_cols = ["date", "district", "dengue_index", "respiratory_index", "rainfall_index"]

    df = df.merge(
        health_signals[health_cols],
        on=["date", "district"],
        how="left",
    )

    # Fill missing health signals with 0
    for col in ["dengue_index", "respiratory_index", "rainfall_index"]:
        if col in df.columns:
            df[col] = df[col].fillna(0)

    logger.info("  Merged health signal features (dengue + respiratory + rainfall)")
    return df
...
def get_feature_columns() -> dict:
    ...
    sku_cols = [
        "therapeutic_category_encoded", "group_encoded", "dosage_group_encoded",
        "price_band_encoded", "country_encoded", "schedule_encoded",
    ...
    health_cols = ["dengue_index", "respiratory_index", "rainfall_index"]
